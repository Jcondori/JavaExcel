...
public class TypeIncome {
    ...
    public static final String SAMPLE_XLSX_FILE_PATH = "C:\\Users\\Optimiza\\Desktop\\Jcondori\\TypeIncome.xlsx";
        ...
        for (Sheet sheet : excel) {
            for (Row row : sheet) {
                if (row.getRowNum() != 0) {
                    ...
                    System.out.println("<record model=\"sunat.type_income\" id=\"" + Numero + "\"><field name=\"number\">" + Numero + "</field><field name=\"description\">" + Descripcion + "</field></record>");
                }
            }
            //System.out.println("------------------------------------------------------");
        }
...
//        guardar(excel);
    ...
    public static void guardar(Workbook wb) throws IOException {
        try {
            ...
            Logger.getLogger(TypeIncome.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
