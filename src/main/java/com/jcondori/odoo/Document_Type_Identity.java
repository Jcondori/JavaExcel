...
public class Document_Type_Identity {

    public static final String SAMPLE_XLSX_FILE_PATH = "C:\\Users\\Jcondori\\Desktop\\JCondori\\document_type_identity.xlsx".replace("\\", "/");
        ...
        for (Sheet sheet : excel) {
            for (Row row : sheet) {
                    if (row.getRowNum() != 0) {
                        ...
                        System.out.println("<record model=\"sunat.document_type_identity\" id=\"" + Numero + "\"><field name=\"number\">" + Numero + "</field><field name=\"description\">" + Descripcion + "</field></record>");
                    }
            }
            //System.out.println("------------------------------------------------------");
        }
...
//        guardar(excel);
    ...
    public static void guardar(Workbook wb) throws IOException {
        try {
            ...
            Logger.getLogger(Document_Type_Identity.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
