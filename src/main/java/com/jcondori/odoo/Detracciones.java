...
public class Detracciones {
    ...
    public static final String SAMPLE_XLSX_FILE_PATH = "C:\\Users\\Jcondori\\Desktop\\JCondori\\Detracciones.xlsx".replace("\\", "/");
        ...
        for (Sheet sheet : excel) {
            for (Row row : sheet) {
                if (row.getRowNum() != 0) {
                    ...
                    String Porcentaje = dataFormatter.formatCellValue(row.getCell(2));
                    Descripcion = Descripcion.trim();
                    System.out.println("<record model=\"sunat.detracciones\"id=\"" + Numero + "\">" +
                            "<field name=\"number\">" + Numero + "</field>" +
                            "<field name=\"description\">" + Descripcion + "</field>" +
                            "<field name=\"detrac\">" + Porcentaje + "</field>" +
                            "</record>");
                }
            }
            //System.out.println("------------------------------------------------------");
...
//        guardar(excel);
    ...
    public static void guardar(Workbook wb) throws IOException {
        try {
            ...
            Logger.getLogger(Detracciones.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
